...
int main(int argc, char ** argv) {
    if (argc < 5) {
        std::cerr << "Invalid arguments, correct format is: " << argv[0] << " size_X size_Y start_x start_Y \"policy file\" (vertical drone distance)" << endl;
        exit(1);
    }
    ...
    if (argc >= 7) {
        sscanf(argv[6], "%f", &verticalDroneDistance);
    }
