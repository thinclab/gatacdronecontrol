...
void GaTACDroneControl::launchClient(char *serverIp, char *serverPort, char *dataPort) {
	...
	struct addrinfo hints, *srv, *info, *datsrv, *dat;
	...
	// Filling 'srv' object with info from 'hints'
	if ((errorCheck = getaddrinfo(host, dp, &hints, &datsrv)) != 0) {
		perror("Client: get address info function.");
		exit(1);
	}

	// Creating socket
	for (info = datsrv; info != NULL; info = info->ai_next) {
		if ((datsock = socket(info->ai_family, info->ai_socktype, info->ai_protocol)) == -1) {
			perror("Client: socket function.");
			continue;
		}
 ...
 * @param toSet String to set this client's navdata value to
 */
string GaTACDroneControl::getBattery(int id)
{
	string line;
 ...
 * @param toSet String to set this client's navdata value to
 */
string GaTACDroneControl::getForwardVelocity(int id)
{
string line;
	ifstream stream0("currentNavdata0.txt");
	ifstream stream1("currentNavdata1.txt");
